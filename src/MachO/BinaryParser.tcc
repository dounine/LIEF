                ...
                switch (DyldInfo::REBASE_OPCODES(opcode)) {
                    ...
                    case DyldInfo::REBASE_OPCODES::DO_REBASE_ULEB_TIMES_SKIPPING_ULEB: {
                        ...
                        for (size_t i = 0; i < count; ++i) {
                            if (current_segment == nullptr) {
                                LIEF_WARN(
                                        "REBASE_OPCODE_DO_REBASE_ULEB_TIMES_SKIPPING_ULEB: the current segment is null");
                            } else if (segment_offset > current_segment->file_size()) {
                                LIEF_WARN(
                                        "REBASE_OPCODE_DO_REBASE_ULEB_TIMES_SKIPPING_ULEB: Bad offset (0x{:x} > 0x{:x})",
                                        segment_offset, current_segment->file_size());
                            }
                            do_rebase<MACHO_T>(type, segment_index, segment_offset, &segments);
...
// ============
        ...
        ok_error_t BinaryParser::parse_fixup_seg(SpanStream &stream, uint32_t seg_info_offset,
            ...
            LIEF_DEBUG("{}pointer_format    = {} ({})", DPREFIX, seg_info.pointer_format,
                       to_string(static_cast<DYLD_CHAINED_PTR_FORMAT>(seg_info.pointer_format)));
            ...
            for (uint32_t page_idx = 0; page_idx < seg_info.page_count; ++page_idx) {
                ...
                if ((offset_in_page & DYLD_CHAINED_PTR_START_MULTI) > 0) {
                    ...
                    while (!chain_end) {
                        ...
                        const uint64_t off = page_start_off +
                                             overflow_index * sizeof(uint16_t); // &segInfo->page_start[overflowIndex]
                        ...
                        if (!is_ok) {
                            LIEF_WARN("Error while walking through the chained fixup of the segment '{}'",
                                      segment->name());
                        }
                        ++overflow_index;
                    }
 ...
 */
   ...
   */
        ...
        ok_error_t BinaryParser::post_process(FunctionStarts &cmd) {
            ...
            if (linkedit == nullptr) {
                LIEF_WARN("Can't find the segment that contains the LC_FUNCTION_STARTS (offset=0x{:016x})",
                          cmd.data_offset());
                return make_error_code(lief_errors::not_found);
            }
        ...
        ok_error_t BinaryParser::post_process(DynamicSymbolCommand &cmd) {
            ...
            for (const std::unique_ptr<Symbol> &sym: binary_->symbols_) {
                ...
                if (cmd.nb_external_define_symbols() > 0 &&
                    cmd.idx_external_define_symbol() <= isym &&
                    isym < (cmd.idx_external_define_symbol() + cmd.nb_external_define_symbols())) {
                    sym->category_ = Symbol::CATEGORY::EXTERNAL;
                }

                if (cmd.nb_undefined_symbols() > 0 &&
                    cmd.idx_undefined_symbol() <= isym &&
                    isym < (cmd.idx_undefined_symbol() + cmd.nb_undefined_symbols())) {
