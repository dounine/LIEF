...
// =======
        ...
        ok_error_t Binary::shift(size_t value) {
            ...
            LIEF_INFO("LC Table wrapped by {} / End offset: 0x{:x} (size: {:x})",
                      load_cmd_segment->name(), loadcommands_end, load_cmd_segment->data_.size());
            load_cmd_segment->content_insert(loadcommands_end, value);//TODO 在space空间那里新增value 0u8字节数据
...
//                LIEF_INFO("cmd offset: {:x} end_offset: {:x} {:}", cmd->command_offset(), loadcommands_end);
        ...
        bool Binary::remove(const LoadCommand &command) {
            ...
            if (auto *lib = cmd_rm->cast<DylibCommand>()) {
                ...
                if (it_cache == std::end(libraries_)) {
                    LIEF_WARN("Library {} not found in cache. The binary object is likely in an inconsistent state",
                              lib->name());
                } else {
            ...
            if (const auto *seg = cmd_rm->cast<const SegmentCommand>()) {
                ...
                if (it_cache == std::end(segments_)) {
                    LIEF_WARN("Segment {} not found in cache. The binary object is likely in an inconsistent state",
                              seg->name());
                } else {
        ...
        LIEF::Binary::functions_t Binary::unwind_functions() const {
            ...
            for (size_t i = 0; i < hdr->index_count; ++i) {
                ...
                if (second_lvl_off > 0 &&
                    vs.can_read<details::unwind_info_regular_second_level_page_header>(second_lvl_off)) {
                    const size_t saved_pos = vs.pos();
                    {
                        vs.setpos(second_lvl_off);
                        const auto lvl_hdr = vs.peek<details::unwind_info_regular_second_level_page_header>(
                                second_lvl_off);
                        if (!lvl_hdr) {
                            break;
                        }
            ...
            const size_t nb_lsda = lsda_stop > lsda_start ? (lsda_stop - lsda_start) /
