...
namespace LIEF {
    namespace MachO {
        ...
        ok_error_t Builder::build(DylibCommand &library) {
            ...
            raw_cmd.compatibility_version = static_cast<uint32_t>(DylibCommand::version2int(
                ...
                for (Symbol &s: binary_->symbols()) {
                    ...
                    switch (s.category()) {
                        case Symbol::CATEGORY::NONE:
                            other_syms.push_back(&s);
                            break;
                        case Symbol::CATEGORY::LOCAL:
                            local_syms.push_back(&s);
                            break;
                        case Symbol::CATEGORY::EXTERNAL:
                            ext_syms.push_back(&s);
                            break;
                        case Symbol::CATEGORY::UNDEFINED:
                            undef_syms.push_back(&s);
                            break;
                ...
                for (const std::unique_ptr<Relocation> &reloc: seg_info.segment.relocations_) {
                    ...
                    switch (fixup.rtypes_) {
                        case RelocationFixup::REBASE_TYPES::ARM64E_AUTH_REBASE: {
                            auto &raw_fixup = *reinterpret_cast<details::dyld_chained_ptr_arm64e *>(sdata.data() +
                                                                                                    rel_offset);
                        ...
                        case RelocationFixup::REBASE_TYPES::PTR64_REBASE: {
                            auto &raw_fixup = *reinterpret_cast<details::dyld_chained_ptr_generic64 *>(sdata.data() +
                                                                                                       rel_offset);
                        ...
                        case RelocationFixup::REBASE_TYPES::PTR32_REBASE: {
                            auto &raw_fixup = *reinterpret_cast<details::dyld_chained_ptr_generic32 *>(sdata.data() +
                                                                                                       rel_offset);
        ...
        ok_error_t Builder::build(DyldChainedFixups &fixups) {
            ...
            switch (fmt) {
                case DYLD_CHAINED_FORMAT::IMPORT:
                    imports.reserve(nb_bindings * sizeof(details::dyld_chained_import));
                    break;
                case DYLD_CHAINED_FORMAT::IMPORT_ADDEND:
                    imports.reserve(nb_bindings * sizeof(details::dyld_chained_import_addend));
                    break;
                case DYLD_CHAINED_FORMAT::IMPORT_ADDEND64:
                    imports.reserve(nb_bindings * sizeof(details::dyld_chained_import_addend64));
                    break;
            }
            ...
            for (const std::unique_ptr<ChainedBindingInfoList> &info: fixups.internal_bindings_) {
                ...
                for (ChainedBindingInfo *binding: info->elements_) {
                    ...
                    switch (binding->btypes_) {
                        ...
                        case ChainedBindingInfo::BIND_TYPES::UNKNOWN:
                            break;
                    }
                }
            }
        ...
        ok_error_t Builder::build(BuildVersion &bv) {
            ...
            const uint32_t raw_size =
                    sizeof(details::build_version_command) + tools.size() * sizeof(details::build_tool_version);
            const uint32_t size_needed = align(raw_size, sizeof(typename T::uint));
            const uint32_t padding = size_needed - raw_size;
