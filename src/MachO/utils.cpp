        ...
        bool check_overlapping(const Binary &binary, std::string *error) {
            for (const SegmentCommand &lhs: binary.segments()) {
                ...
                for (const SegmentCommand &rhs: binary.segments()) {
                    ...
                    const bool vm_overalp =
                            (rhs.virtual_address() <= lhs.virtual_address() && rhs_vm_end > lhs.virtual_address() &&
                             lhs_vm_end > lhs.virtual_address()) ||
                            (rhs.virtual_address() >= lhs.virtual_address() && rhs.virtual_address() < lhs_vm_end &&
          ...
          Segments '{}' and '{}' overlap (virtual addresses):
            [0x{:08x}, 0x{:08x}] [0x{:08x}, 0x{:08x}]
                    ...
                    const bool file_overlap =
                            (rhs.file_offset() <= lhs.file_offset() && rhs_file_end > lhs.file_offset() &&
                             lhs_file_end > lhs.file_offset()) ||
                            (rhs.file_offset() >= lhs.file_offset() && rhs.file_offset() < lhs_file_end &&
                             rhs_file_end > rhs.file_offset());
                    if (file_overlap) {
                        if (error) {
                            *error = fmt::format(R"delim(
