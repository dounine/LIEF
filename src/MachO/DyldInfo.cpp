...
namespace LIEF {
    namespace MachO {
        ...
        std::string DyldInfo::show_rebases_opcodes() const {
            ...
            size_t pint_v = static_cast<LIEF::Binary *>(binary_)->header().is_64() ? sizeof(uint64_t)
                                                                                   : sizeof(uint32_t);
            ...
            while (!done && rebase_stream.pos() < rebase_stream.size()) {
                ...
                switch (opcode) {
                    ...
                    case REBASE_OPCODES::ADD_ADDR_IMM_SCALED: {
                        ...
                        output << "[" << to_string(opcode) << "]";
                        output << "Segment Offset += " << std::hex << std::showbase << (imm * pint_v) << " ("
                               << segment_offset << ")";
                        output << '\n';
                        break;
                    }
                    ...
                    case REBASE_OPCODES::DO_REBASE_ADD_ADDR_ULEB: {
                        ...
                        output << tab;
                        output << "Segment Offset += " << std::hex << std::showbase << (val + pint_v) << " ("
                               << segment_offset << ")";
                        output << '\n';
                    ...
                    case REBASE_OPCODES::DO_REBASE_ULEB_TIMES_SKIPPING_ULEB: {
                        ...
                        for (size_t i = 0; i < count; ++i) {
                            ...
                            output << tab << tab;
                            output << "Segment Offset += " << std::hex << std::showbase << skip << " + " << pint_v
                                   << " (" << segment_offset << ")";
                            output << '\n' << '\n';
                        }
                ...
                switch (opcode) {
                    ...
                    case BIND_OPCODES::DO_BIND: {
                        if (!use_threaded_rebase_bind) {
                            ...
                            output << (library_ordinal > 0 ? libraries[library_ordinal - 1].name() : std::to_string(
                                    library_ordinal));
                            output << ", addend=";
                            ...
                            output << tab << "Segment Offset += " << std::hex << std::showbase << pint_v << " ("
                                   << segment_offset << ")" << '\n';
                        } else {
                            ordinal_table.push_back(
                                    ThreadedBindData{symbol_name, addend, library_ordinal, symbol_flags, type});
                        }
                        break;
                    }
                    ...
                    case BIND_OPCODES::DO_BIND_ADD_ADDR_ULEB: {
                        ...
                        output << tab << "Segment Offset += " << std::hex << std::showbase << v + pint_v << " ("
                               << segment_offset << ")" << '\n';
                        break;
                    }
                    ...
                    case BIND_OPCODES::DO_BIND_ADD_ADDR_IMM_SCALED: {
                        ...
                        output << tab << "Segment Offset += " << std::hex << std::showbase << (imm * pint_v + pint_v)
                               << " (" << segment_offset << ")" << '\n';
                        ...
                        switch (subopcode) {
                                ...
                                do {
                                    ...
                                    if (is_rebase) {
                                        ...
                                        if (ordinal >= ordinal_table_size || ordinal >= ordinal_table.size()) {
                                            LIEF_WARN(
                                                    "bind ordinal ({:d}) is out of range (max={:d}) for disk pointer 0x{:04x} in "
                                                    "segment '{}' (segment offset: 0x{:04x})", ordinal,
                                                    ordinal_table_size, value,
                                                    current_segment.name(), segment_offset);
                                            break;
                                        }
                                        if (address < current_segment.virtual_address() ||
                                            address >=
                                            (current_segment.virtual_address() + current_segment.virtual_size())) {
                                        ...
                                        output << tab << tab << fmt::format(
                                                "threaded_bind({}/{}, 0x{:x}, {}, {}, library_ordinal={}, "
                                                "addend={}, is_weak_import={})\n",
                                                "THREADED_BIND", to_string(DyldBindingInfo::TYPE(th_bind_data.type)),
                                                segment_offset,
                                                current_segment.name(), th_bind_data.symbol_name,
                                                library_ordinal > 0 ? libraries[library_ordinal - 1].name()
                            ...
                            case BIND_SUBOPCODE_THREADED::SET_BIND_ORDINAL_TABLE_SIZE_ULEB: {
                                ...
                                if (auto res = bind_stream.read_uleb128()) {
                                    ...
                                    LIEF_ERR(
                                            "Can't read BIND_SUBOPCODE_THREADED_SET_BIND_ORDINAL_TABLE_SIZE_ULEB count");
                                    break;
                                }
                                ordinal_table_size = count +
                                                     1; // the +1 comes from: 'ld64 wrote the wrong value here and we need to offset by 1 for now.'
...
// ===========
        ...
        DyldInfo &DyldInfo::update_rebase_info(vector_iostream &stream) {
            ...
            for (RelocationDyld *rebase: rebases) {
                ...
                if (address != rebase->address()) {
                    if (rebase->address() < current_segment_start || rebase->address() >= current_segment_end) {
                        ...
                        output.emplace_back(static_cast<uint8_t>(REBASE_OPCODES::SET_SEGMENT_AND_OFFSET_ULEB),
            ...
            for (auto it = std::begin(output); it->opcode != REBASE_OPCODES::DONE; ++it) {
                ...
                if (it->opcode == REBASE_OPCODES::ADD_ADDR_ULEB && it->op1 < (15 * pint_size) &&
                    (it->op1 % pint_size) == 0) {
            ...
            for (DyldBindingInfo *info: bindings) {
                ...
                if (sym != nullptr) {
                    if (sym->name() != symbol_name) {
                        ...
                        instructions.emplace_back(uint8_t(BIND_OPCODES::SET_SYMBOL_TRAILING_FLAGS_IMM), flag, 0,
                                                  sym->name());
                ...
                if (info->address() != address) {
                    if (info->address() < current_segment_start || current_segment_end <= info->address()) {
                        ...
                        instructions.emplace_back(static_cast<uint8_t>(BIND_OPCODES::ADD_ADDR_ULEB),
            ...
            for (auto it = std::begin(instructions); !done && it != std::end(instructions); ++it) {
                ...
                switch (static_cast<BIND_OPCODES>(inst.opcode)) {
                    ...
                    case BIND_OPCODES::SET_DYLIB_SPECIAL_IMM: {
                        raw_output
                                .write<uint8_t>(
                                        uint8_t(BIND_OPCODES::SET_DYLIB_SPECIAL_IMM) | (inst.op1 & IMMEDIATE_MASK));
                        break;
                    }
        ...
        DyldInfo &
        DyldInfo::update_standard_bindings(const DyldInfo::bind_container_t &bindings, vector_iostream &stream) {
        ...
        DyldInfo &
        DyldInfo::update_standard_bindings_v1(const DyldInfo::bind_container_t &bindings, vector_iostream &stream) {
        ...
        DyldInfo &DyldInfo::update_standard_bindings_v2(const DyldInfo::bind_container_t &bindings_set,
            ...
            std::sort(std::begin(threaded_rebase_bind_indices), std::end(threaded_rebase_bind_indices),
                      [&bindings, &rebases](int64_t index_a, int64_t index_b) {
                          ...
                          uint64_t address_a =
                                  index_a <= 0 ? rebases[-index_a]->address() : bindings[index_a - 1]->address();
                          uint64_t address_b =
                                  index_b <= 0 ? rebases[-index_b]->address() : bindings[index_b - 1]->address();
            ...
            for (auto it = std::begin(instructions); !done && it != std::end(instructions); ++it) {
                ...
                switch (static_cast<BIND_OPCODES>(it->opcode)) {
                    ...
                    case BIND_OPCODES::THREADED_SET_BIND_ORDINAL_TABLE_SIZE_ULEB: {
                        raw_output
                                .write<uint8_t>(
                                        static_cast<uint8_t>(BIND_OPCODES::THREADED_SET_BIND_ORDINAL_TABLE_SIZE_ULEB))
                                .write_uleb128(inst.op1);
                        break;
                    }
