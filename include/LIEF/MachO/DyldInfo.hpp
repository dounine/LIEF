...
/// Class that represents the LC_DYLD_INFO and LC_DYLD_INFO_ONLY commands
        ...
        private:
            using bind_container_t = std::set<DyldBindingInfo *, std::function<bool(DyldBindingInfo *,
                                                                                    DyldBindingInfo *)>>;

            void show_bindings(std::ostream &os, span<const uint8_t> buffer, bool is_lazy = false) const;

            void show_trie(std::ostream &output, std::string output_prefix, BinaryStream &stream, uint64_t start,
                           uint64_t end, const std::string &prefix) const;
